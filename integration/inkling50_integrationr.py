...
class Notepad:
    ...
    def __bindEvents(self):
        self.__root.bind("<KeyPress>", self.__onKeyPress)
    ...
    def __onKeyPress(self, event):
        # Always send "Timer Reset" on keypress
        self.pipe.send("Timer Reset")  
        self.__resetIdleTimer()        # Always reset the idle timer
        # Reset idle state to active after a keypress
        if self.expired_idle:
            self.expired_idle = False

    def __expiredIdle(self):
        # Only send "Idle Expired" if the system just entered the idle state
        if not self.expired_idle:
            self.expired_idle = True   # Mark the system as idle
    ...
    def __resetIdleTimer(self):
        if self.idle_timer is not None:
            try:
                ...
                print(f"Error cancelling idle timer: {e}")
        self.idle_timer = self.__root.after(self.idle_timelimit, self.__expiredIdle)
