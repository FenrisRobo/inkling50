...
- Can I save the notepad's edited/customized text in the PDF?
  - To clarify, the notepad's text-editing features are mainly for the user to use *while* they are typing. As such, they are not intended to be saved for later recall by the user. They save as a PDF file in case there is something that the user absolutely needs to check. Furthermore, in the future, the 'save as PDF' feature will be used as a means to collect metadata from the user to make inkling50 more dynamic and interactive, such as integrating with the calendar.
